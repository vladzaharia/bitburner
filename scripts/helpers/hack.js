...
export async function hackWeakenGrow(ns, hostname) {
	...
	if (secLevel < MIN_SEC_LEVEL) {
		...
		return await ns.hack(hostname).then(async (amtMoney) => {
			if ((amtMoney === 0) && (secLevel > secThresh)) {
				return await ns.weaken(hostname);
			} else if ((amtMoney > 0) && (moneyAvail < moneyThresh) && (moneyAvail < 100000)) {
				return await ns.grow(hostname);
			}
		});
	}
